...
//        $addbtn .= \html_writer::link(new \moodle_url('/blocks/vavt_ref_company/adding.php', ['action' => 'add']), 'Добавить предприятие<i class="fa fa-plus-circle" aria-hidden="true" style="font-family: FontAwesome"></i>',
        $addbtn .= \html_writer::link(new \moodle_url('/blocks/vavt_ref_company/set_company.php'), 'Управление компаниями <i class="fa fa-cog" aria-hidden="true" style="font-family: FontAwesome"></i>',
//            array('type' => 'button', 'class' => 'btn btn-outline-primary')
    ...
    foreach ($data as $d) {
        ...
        $readlnk = \html_writer::link(new \moodle_url('/blocks/vavt_ref_company/view.php', ['id' => $d->id]), 'Подробнее',
        ...
        if ($typenews == 'block') {
            ...
            $itemlnk = \html_writer::link(new \moodle_url('/blocks/vavt_ref_company/view.php', ['id' => $d->id]), $d->name,
                array('target' => '_blank'));
...
//                NULL);
            ...
            $params['picture'] = true;
        } else $imgsrc = $CFG->wwwroot.'/blocks/vavt_ref_company/templates/itemimg.png';

        $match[$i] = [
            'name' => $itemlnk,
            'text' => trimString($params['content']),
            'user' => $userlnk,
            'tagregion'=> $d->tagregion,
            'tagindustry'=> $d->tagindustry,
        ...
        if (is_siteadmin()) {
            $editlnk = \html_writer::link(new \moodle_url('/blocks/vavt_ref_company/adding.php', ['action' => 'edit', 'id' => $d->id]), '<i class="fa fa-pencil-square-o" aria-hidden="true" style="font-family: FontAwesome; margin-left: 15px;"></i>');
            ...
            $dellnk = \html_writer::link(
                new \moodle_url('/blocks/vavt_ref_company/deleteitem.php', ['id' => $d->id, 'action' => 'deleteitem', 'contextid'=>$context->id]),
