...
if($id <> 0 && $action == 'deleteitem'){
    ...
    $fs->delete_area_files($contextid, 'block_vavt_project', 'pictures', $id);

    $DB->delete_records('block_vavt_project', array('id' => $id));
    $text = 'Элемент удален';
    $alertClass = 'm-t-2 alert alert-info';

//$redirectLink = new \moodle_url($PAGE->url, ['action' => 'editscreen', 'screenid' => $screenID]);

    $redirectLink = new \moodle_url('/blocks/vavt_project/index.php');
