...
$PAGE->set_url('/blocks/vavt_project/view.php');
$PAGE->set_title("Проекты сообщества");

$data = $DB->get_record("block_vavt_project",  ['id' => $id]);

$PAGE->set_heading($data->name);
$PAGE->navbar->add('Проекты сообщества', new \moodle_url('/blocks/vavt_project/index.php'));
...
$params['content'] = file_rewrite_pluginfile_urls($params['content'], 'pluginfile.php', $context->id, 'block_vavt_project', 'content', $id);
...
//$files = $fs->get_area_files($context->id, 'block_vavt_project', 'pictures');
...
if ( !empty($picture) )
{
    ...
    $imgsrc = $CFG->wwwroot.'/blocks/vavt_project/templates/itemimg.png';
...
if($DB->record_exists('vavt_favorite', ['usermodified' => $USER->id,  'nameplugin' => 'project', 'objid'=>$id])){
    $render['has_addfav'] = 'addfav';
}

echo $OUTPUT->render_from_template("block_vavt_project/item", $render);
